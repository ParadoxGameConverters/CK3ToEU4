...
class LocalizationMapper
{
  public:
	...
	void unravelNestedLocs(LocBlock& block)
	...
	void removeEmphasis(LocBlock& block)
	{
		for (const auto& lang: std::vector<std::string>{"english", "french", "spanish", "german"})
		{
			const auto& loc = selectLanguage(lang, block);
	...
	[[nodiscard]] std::optional<LocBlock> getLocBlockForKey(const std::string& key)
	{
		if (key.empty())
			return std::nullopt;
	...
	[[nodiscard]] std::optional<std::string> reverseLookupCultureName(const std::string& localization)
	{
  ...
  private:
	...
	const std::string selectLanguage(const std::string& language, const LocBlock& block)
	{
		if (language == "english")
	...
	void assignSelectLanguage(const std::string& str, const std::string& language, LocBlock& block)
	{
		if (language == "english")
	...
	private static std::pair<std::string, std::string> determineKeyLocalizationPair(const std::string& text)
	{
